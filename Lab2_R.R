...
rep(c(1, 2, 3), 10)  #replicate element  ทำช้ำ
?rep
help(rep)
help(rnorm)
?seq
...
sortd.rusult <- sort(v3,decreasing = TRUE) 
print(sortd.rusult)


vec4 <- c(1, 2, 3, "one", "two", "three", TRUE, FALSE)
print(vec4)
...
matrix(5, nrow=1) #1*1dim
matrix(1:4, nrow=2) #2*21dim
matrix(1:4, nrow=2, byrow=TRUE) #2*21dim

matrix(1:9, nrow=3, ncol = 3) #2*21dim
matrix(1:9, nrow=3, byrow= TRUE) #2*21dim

matrix(1:6, nrow=3) #3*21dim
...
chind(my.mat, my.mat)    #column comhine
rhind(my.mat, my.mat)    #row comhine
...
# mul matrices
mat.result <- mat1*mas2
cat("Result of Multiplitction","\n")
print(mat.result)

# div matrices
mat.result <- mat1-mas2
cat("Result of divtraction","\n")
print(mat.result)
...
m.df <- dataframe(cities, counties, population)
print(m.df)


emp.data <- data.frame(
  emp_id = c(1:5),
  emp_name = c("Reck","Dan","Mic","Ryan","Gray"),
  salary = c(500,600,700,800,900),
  start_date = as.Date(c("2012-02-15","2012-04-13","2015-10-03","2017-05-15",
                       "2021-07-04")))
print(emp.data)
str(emp.data)  # get structure data frame

# extract column
resut.emp <- data.frame(emp.data$emp_name,emp.data$salary)
print(result.emp)
...
resuit.emp3 <- emp.data[c(3,5),c(2,4)]
print(resuit.emp3)

# add column in data fream
...
emp.data2 <- data.frame(
  ...
  start_date = as.Date(c("2012-02-15","2012-04-13","2015-10-03"),
  ))
emp.data2$dept = c("Operator","IT","Hr")
